...
class SIREN_with_rotation(nn.Module):
    """SIREN Module
    ...
    def __init__(self, n_in_features, n_out_features, hidden_layer_config=[],
                 w0=30, ww=None, delay_init=False, freeze_network_weights=False, freeze_rotation_weights=False):
        super(SIREN_with_rotation, self).__init__()
        ...
        self.rotation_matrix = RotationMatrix(freeze_rotation_weights)
        self.freeze_network_weights = freeze_network_weights
    ...
    def forward(self, x, preserve_grad=False):
        ...
        coords = self.rotation_matrix(coords_org)
        if self.freeze_network_weights:
            with torch.no_grad():
                y = self.net(coords)
        else:
            y = self.net(coords)
...
def from_state_dict(weights: OrderedDict, device: str = "cpu", w0=1, ww=None, freeze_network_weights=False, freeze_rotation_weights=False):
    ...
    model = SIREN_with_rotation(
        n_in_features=n_in_features,
        n_out_features=n_out_features,
        hidden_layer_config=hidden_layer_config,
        w0=w0, ww=ww, delay_init=True, freeze_network_weights=freeze_network_weights,
        freeze_rotation_weights=freeze_rotation_weights
    )
